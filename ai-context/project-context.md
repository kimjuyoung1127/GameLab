...
## 1) 한 줄 정의
AI가 먼저 음향 이상 구간을 제안하고, 사용자가 검수(C/X/Shift+F)와 수동 구간 생성(Box), 스펙트로그램 분석 도구(리스닝/뷰포트 포커스/줌)로 라벨 결과를 완성하는 협업형 음향 라벨링 플랫폼.
...
4. Spectrogram Analysis: 영역 선택 리스닝(원본/필터링) + 박스 클릭 전체 Fit/`BAND` 포커스 + 줌/커서
...
## 11) 스펙트로그램 분석 기능 (Sprint 14 / 14.1 현재 기준)
...
- 0.25x~2.0x 재생 속도 범위
- PNG 스크린샷 내보내기
- 제거된 UI: LIN/LOG 축 토글, FFT 설정 패널, 분석/저/중/고 주파수 프리셋
